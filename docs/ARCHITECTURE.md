...
| **P5 Evals** | M | Seed cases per §11.3. **`scripts/build_eval_fixtures.py` dropped** (advisor review): its only job would have been copying four files already hand-verified and open in the working transcript into `tests/fixtures/stubs/` once -- a generator with no second consumer, same call as dropping P3's `docs.py`/`oca.py`. Stub trees are hand-written mimics per the existing `tests/fixtures/stubs` convention instead. First slice shipped: `po-approval-threshold` (config trap), `quality-check-ce` (red/trap, the 100%-target case), `ambiguous-need` (rung 0, no scaffold at all) -- chosen to exercise every eval mechanism (scaffold+profile, `checkbox search` over a copied stub, `tool_used` trap grader, `regex` on `trace`, and the no-scaffold path) before writing the other nine mechanically | `claude plugin eval plugins/checkbox --case 'po-*' --runs 1 --ablation none --allow-tools Write Edit Bash` (iterate) · full run (§11.1) | §11.2 targets met; report archived in `docs/benchmarks/`. Smoke-tested 2026-09-15: `ambiguous-need` ran end to end ($0.059, no Bash needed) and surfaced a real calibration finding -- the agent asked 3 clarifying questions, not the rubric's "exactly one" |
