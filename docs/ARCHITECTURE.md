...
| **P5 Evals** | M | All 12 §11.3 seed cases shipped. **`scripts/build_eval_fixtures.py` dropped** (advisor review): its only job would have been copying a handful of files already hand-verified and open in the working transcript into `tests/fixtures/stubs/` once -- a generator with no second consumer, same call as dropping P3's `docs.py`/`oca.py`. Stub trees are hand-written mimics per the existing `tests/fixtures/stubs` convention instead. First 3 cases (`po-approval-threshold`, `quality-check-ce`, `ambiguous-need`) exercised every eval mechanism -- scaffold+profile, `checkbox search` over a copied stub, `tool_used` trap grader, `regex` on `trace`, `llm` rubric grader, and the no-scaffold path -- before the other 9 were written mechanically against the same templates | `claude plugin eval plugins/checkbox --case 'po-*' --runs 1 --ablation none --allow-tools Write Edit Bash` (iterate) · full run (§11.1) | §11.2 targets met; report archived in `docs/benchmarks/`. Smoke-tested 2026-09-15: `ambiguous-need` ran end to end ($0.059, no Bash needed) and surfaced a real calibration finding -- the agent asked 3 clarifying questions, not the rubric's "exactly one". All 12 cases confirmed to load with zero errors at $0 (`--max-cost-usd 0`). The other 11 cases need `Bash` granted (`socat` missing on the dev machine as of this writing) -- **not yet run** |
