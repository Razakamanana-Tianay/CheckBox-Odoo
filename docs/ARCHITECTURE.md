...
- **Priority order:**
  ...
  3. Codex (plugin hooks, the same pattern Ponytail uses) -- **built in P6 only as a documented deferral**, not shipped: a full Codex plugin is a materially bigger lift than a rules file, for a host that (like every entry below) never gets the guard, and no real user had asked for it as of P6. Revisit `adapters/CLAUDE.md`'s Priority note if that changes;
