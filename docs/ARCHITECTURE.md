...
- **Verification:** this table lives as prose in `rules/hosting.md`, loaded on demand by the `ladder` skill. A machine-readable `rules/hosting.json` is deferred until something other than a skill's own on-demand read needs it (YAGNI, per the repo's own non-negotiable #8) -- see `plugins/checkbox/rules/CLAUDE.md`. Re-check the underlying facts at each major Odoo release either way.
...
| `docs` (not built) | `odoo/documentation`, branch = version, `content/applications/**` | Sparse shallow clone, sections split by heading | `$DATA/docs/<version>.sqlite` |
| `oca` (not built) | Curated OCA repositories, branch = version | Shallow clone, manifest fields (name, summary, development_status, maintainers) + last commit date | `$DATA/oca/<version>.sqlite` |
| `live` (not built) | Any read-only Odoo MCP server | Installed modules, settings values | Not cached |

`docs` and `oca` were deferred at P3 (`knowledge/search.py`'s own docstring) and stayed deferred through P7 (D10): no seed case's evidence needs them yet, and both are real network+disk cost. `live` was explicitly P7 scope but scoped out at D10 -- "any read-only Odoo MCP server" names no concrete server to build or verify a client against. `search`/`classify`/`card_validate` are real and shipped (P3, P4, P7) as both CLI subcommands and `checkbox-mcp` tools.
...
├── rules/                      # ladder.md (canonical), ladder.compact.md, hosting.md, risk/*.json
...
- **Generation:** `scripts/build_adapters.py` renders `rules/ladder.md` into:
  ...
  - `adapters/opencode/` -- since P9/v1.1.0 a ready-to-install **npm plugin**
    package: `package.json`, `src/index.ts` (always-on ladder injection + a
    `checkbox` tool shelling to the bundled CLI + blast-radius tagging after
    writes), `src/ladder.ts` (neutral ladder as a TS string) and `vendor/` (a
    verbatim copy of the stdlib-only core; the plugin runs it with plain
    `python3`, no install step).
- **One-command install:** `checkbox setup <host> --project <dir> [--dry-run]`
  copies/merges the generated rule files into a project for `opencode`
  (`AGENTS.md`, merged not clobbered), `cursor`, `windsurf` and `copilot`.
  The opencode npm plugin covers the "no file to copy" case -- `opencode2
  plugin add checkbox-odoo` or a `"plugin": ["file:…/adapters/opencode"]`
  entry in `opencode.json`.
- **Drift check:** `scripts/check_rule_copies.py` fails CI if any adapter
  (including the vendored core and the npm package files) differs from a
  fresh render.
- **Priority order:**
  1. Claude Code (full plugin);
  2. OpenCode (npm plugin + AGENTS.md + CLI, P9) -- the plugin reminds and
     classifies, it still cannot deny edits (OpenCode has no deny-on-write);
  3. Codex (plugin hooks, the same pattern Ponytail uses) -- **built in P6
     only as a documented deferral**, not shipped: a full Codex plugin is a
     materially bigger lift than a rules file, for a host that (like every
     entry below) never gets the guard, and no real user had asked for it as
     of P6. Revisit `adapters/CLAUDE.md`'s Priority note if that changes;
  4. rules-file-only hosts (Cursor, Windsurf, Copilot -- shipped in P6,
     installed by `checkbox setup` in P9).
...
|---|---|---|---|---|
| **P0 Explore** | S | Verified hook I/O, the Stop loop flag, SubagentStart output, file-tool names, scaffold_script cwd/env, marketplace schema. Ponytail structure notes (hooks, skills, drift check, benchmarks) | `test -n "$ODOO18_SRC"` (placeholder check) | Every item in §15 answered, with a doc link |
...
| **P6 Adapters** | S/M | `scripts/build_adapters.py` renders `rules/ladder.md` into `adapters/{AGENTS.md, cursor/checkbox.mdc, windsurf/checkbox.md, copilot/copilot-instructions.md, opencode/README.md}` with neutral (never baked-in) placeholders; `scripts/check_rule_copies.py` drift gate (in-memory diff, no temp dir). Host conventions verified against official docs (Cursor, Copilot) and against a real shipping multi-host plugin's own adapters (Ponytail's `docs/agent-portability.md`, for Windsurf's path and the AGENTS.md-reader host list) rather than guessed. **Codex's fuller plugin-hooks treatment deliberately deferred** (see `adapters/CLAUDE.md`'s Priority note) -- no real user has asked for it yet, and it's a materially bigger lift (an actual plugin manifest + hook shim) for a host that gets no guard either way. `rules/hosting.json` (referenced in several docs as if built) turned out to be a stale forward-reference from the original P0 skeleton, never actually needed by any consumer beyond the `ladder` skill's own on-demand read of the prose `hosting.md` -- corrected the references rather than build an unneeded file | `python3 scripts/build_adapters.py && python3 scripts/check_rule_copies.py` · `pytest -q tests/test_build_adapters.py` | Drift check green (verified); 16 tests covering placeholder rendering, per-host frontmatter, the 12,000-char Windsurf limit, and two real bugs caught while building this: a bare `#` line renders as a Markdown heading outside frontmatter (not a comment), and short inline placeholders (`{version}`/`{edition}`) need short neutral values or the surrounding sentence's grammar breaks |
| **P7 Reach** (optional) | L | `checkbox-mcp` server (`lib/checkbox/mcp_server.py`, the one file allowed a lazy third-party import): `search`, `classify`, `card_validate` as MCP tools, registered via `plugins/checkbox/.mcp.json` pointed at a `$CLAUDE_PLUGIN_DATA` venv (not auto-installed -- see `plugins/checkbox/CLAUDE.md`'s `.mcp.json` section). 19.0 risk overlay, verified the same way as 17.0/18.0. **Scoped down on review (D10):** the OCA index and `live` evidence through an external Odoo MCP server were not built -- see D10 for why | `pytest -q tests/test_mcp_server.py` (5 tests: a static AST guard that `mcp` is never imported at module top level, a real subprocess against a system interpreter genuinely without the SDK confirming a clean exit 1 + message instead of a traceback, and 3 tests exercising `search`/`classify`/`card_validate` through the SDK's own in-process `Client`) · `checkbox rules verify --version 19.0 --odoo-src <sparse 19.0 clone>` (verified, exit 0, same 3 models/5 methods as 18.0, no renames found) | Non-Claude hosts can call `search`/`classify`/`card_validate` -- verified for real: a genuine subprocess speaking the real stdio transport (`mcp.Client(StdioServerParameters(...))`, not just the in-process test above) listed all 3 tools and got a real `search` hit back. The MCP Inspector's own web UI (`mcp dev`) was not driven interactively in this environment (no display); the stdio-subprocess check above exercises the identical protocol path it wraps |
| **P8 Publish** | S | Root `README.md` written with the benchmark table (11/12 cases at 1.0, `ambiguous-need` and `quality-check-ee` caveats as footnotes), install and "how it works" sections, MIT, and Ponytail credit per D8. Marketplace listing finalized (`.claude-plugin/marketplace.json`: `checkbox-odoo` marketplace, `checkbox` plugin). A post-ready LinkedIn draft that answers "where do you draw the line" with the P5 numbers (10/12 cases resolve below the code rung — only `port-of-loading` and `quality-check-ce` land on code — and 100% red recall). Plugin bumped `0.1.0` → `1.0.0` in `plugins/checkbox/.claude-plugin/plugin.json` for first public release; the bump drifts the committed adapters (they embed the plugin version), so `scripts/build_adapters.py` was re-run and the version-only diffs committed with the release | `claude plugin validate . --strict` (exit 0) · `claude plugin validate plugins/checkbox` (exit 0, one accepted warning) · full fast-checks: `ruff check .`+`format --check` (exit 0), `pytest -q` 129 passed / 1 skipped, `python3 scripts/check_rule_copies.py` (drift green) | Public repo: README that reads like a product landing page, a strict-validating marketplace manifest, a post-ready draft, and a reproducible benchmark referenced in both README and post |
| **P9 One-command install** | M | `checkbox setup <host> --project DIR [--dry-run] [--json]` (`lib/checkbox/setup.py` + `cli.py`) for `opencode`/`cursor`/`windsurf`/`copilot`: copies or merges the P6 adapter files into the project, idempotent (AGENTS.md merges instead of clobbering; a re-run skips). The opencode **npm plugin package** becomes another generated adapter set under `adapters/opencode/` (package.json, `src/index.ts`, `src/ladder.ts`, vendored `vendor/` = verbatim `bin/checkbox`+`lib/checkbox/*.py`+`rules/**`, plus `AGENTS.md`): always-on ladder via `experimental.chat.system.transform`, a `checkbox` tool shelling to the bundled stdlib-only CLI with plain `python3` (no install step), and PostToolUse-style tier tagging via `tool.execute.after` -- reminded/classified, still never a gate (OpenCode has no deny-on-write). Plugin hook API verified against the published `@opencode-ai/plugin` v1.18.31 type surface (registry.npmjs.com + unpkg `dist/index.d.ts`/`dist/tool.d.ts`, 2026-09-15), not guessed -- the plugin itself can't be executed in this repo (no node/bun), so a manual smoke checklist stands in `adapters/opencode/README.md`. MCP auto-registration was deliberately NOT added to the plugin (D11); the optional P7 MCP server is untouched. Plugin bumped `1.0.0` → `1.1.0`; adapters regenerated | `pytest -q tests/test_setup.py tests/test_build_adapters.py` (11 new + 5 extended tests: write/overwrite/merge/skip per host, `--dry-run`, sha report, verbatim-vendor identity, package-json version sync, drift detection in vendored files) · `python3 scripts/check_rule_copies.py` (drift green) · fast-checks (ruff, full `pytest -q` 146 passed / 1 skipped) | `setup` installs all four hosts correctly from a fresh project, merges an existing AGENTS.md, and is idempotent (verified by the tests and a manual CLI smoke); the vendored CLI's `doctor` and `classify` run from `adapters/opencode/vendor/` against the fixtures (smoke-tested); the bundled core is byte-identical to `plugins/checkbox/` (test-enforced) and stays stdlib-only. Human-side smoke test to complete before using the plugin for real: run it once under opencode (npm package name `checkbox-odoo` must exist on the registry at publish time) |
...
| D9 | Eval fixtures are hand-written stubs + `scaffold.sh`, not a generator script | A concurrent, uncoordinated session independently built `scripts/build_eval_fixtures.py` + per-case `fixture/` dirs (following a stale nested `evals/CLAUDE.md` never updated since the P1 import) while this session built stub+scaffold.sh. Neither committed `case.yaml`/`scaffold.sh` ever referenced `fixture/`, so the generator's output was unconsumed by any real run. User's explicit decision, 2026-09-15: keep stub+scaffold.sh. History rewritten to drop the 6 generator commits (`f798137`..`d87934a`) rather than leave an add-then-revert trail in public history | A generator gains a real second consumer beyond its own drift check (e.g. another tool needs the sliced-fixture format) |
| D10 | P7 shipped as `checkbox-mcp` (search/classify/card_validate) + the 19.0 risk overlay only; the OCA index and `live` evidence through an external Odoo MCP were scoped out, not just deferred silently | OCA: curated-repo shallow clones are real network+disk cost (16GB free when this was written) for evidence no seed case needs yet -- same YAGNI reasoning as `hosting.json`/`docs.py` (§7.1, §9). `live`: "any read-only Odoo MCP server" names no concrete server to build or test against in this environment; building a client for an unspecified protocol partner risks shipping untested guesswork, which contradicts "no Odoo fact without a source." Both rows stay in §7.1/§13 as future work, not removed. | A real user names a specific OCA module search need, or a specific read-only Odoo MCP server to integrate `live` evidence against |
| D11 (P9) | The OpenCode plugin ships as generated adapters (`adapters/opencode/`, produced by `build_adapters.py`), and the opencode plugin deliberately does NOT register the MCP server or touch `opencode.json` | Plugin-adjacent code must live under the single-source-of-truth generator so `check_rule_copies.py` proves it, exactly like the rules-file adapters. MCP auto-registration would mean either mutating the user's `opencode.json` (unverifiable guesswork about the plugin config hook) or requiring a venv on install -- both contradict the stdlib-only, no-install-step core; `checkbox setup opencode` plus the bundled `checkbox` tool already give non-Claude users the essentials. | A real user asks for MCP tools inside opencode (the P7 server is already usable from any host via generic MCP client config) |
| D12 (v1.1.1) | `hooks/hooks.json` switched from exec form (`"command": "python3"`, `"args": [...]`) to shell form calling a new `bin/checkbox-hook` bash launcher, which walks `$PATH` by hand to find a real `python3`/`py -3`/`python`, skipping any match under a `WindowsApps` path segment | Confirmed live on a real Windows install (Python 3.12 present at a real path, but nothing on PATH answered to `python3` except the Microsoft Store's placeholder alias stub -- python.org's Windows installer only provides `python.exe`, and the stub is a real executable file, so `command -v`/PATH resolution alone can't tell it apart from a working interpreter). The first attempt at a fix was rejected after verifying real constraints, not for lack of trying: exec form is documented as no-shell, so no `\|\|` fallback chain is possible there. The `"shell"` hook field (verified against code.claude.com/docs/en/hooks, 2026-09-16) unblocked it: omitting `shell` already defaults to bash on Windows whenever Git Bash is installed, falling back to PowerShell only when it genuinely isn't -- so shell-form + a hand-walked `$PATH` loop (skipping `WindowsApps` matches, since that's the one stable, documented marker for the stub) works on the common case without needing POSIX/PowerShell-portable syntax at all. A per-OS `commandWindows` field exists in some hook schemas but isn't supported by the Claude Code marketplace validator (corroborated by Ponytail's own `tests/hooks-windows.test.js`, written after its issues #19 and #593 hit this exact class of bug) -- not needed here since the default-shell behavior already does the platform split. `bin/checkbox-hook` derives its own directory from `${BASH_SOURCE[0]}` rather than `$CLAUDE_PLUGIN_ROOT` (that substitution doesn't reach inside a script the hook command merely invokes) and via bash parameter expansion rather than the external `dirname` command, since the whole point of this script is coping with a `$PATH` that's missing things it should have. Remaining gap, not a regression: a Windows machine with no Git Bash at all still fails exactly as before (PowerShell is used, this script never runs) -- fail-open, no worse than pre-fix. Tested hermetically in `tests/test_checkbox_hook_launcher.py` (fake interpreters simulating the stub, no real Windows box needed) plus one true end-to-end case against the real interpreter and real `bin/checkbox`. The OpenCode npm plugin has the identical latent problem in `adapters/opencode/src/index.ts`'s `runCli` (it never goes through `hooks.json` at all, so it needed its own copy, not a shared fix): `findPython()`/`resolveOnPath()` there reimplement the same PATH-walk-skip-WindowsApps-then-`py`-then-`python` logic in TypeScript, since Bun's `spawn` also runs with no shell. Untestable end-to-end in this environment (no `bun` available -- same limitation as the rest of the npm plugin, ARCHITECTURE §13's P9 row); verified only via `node --experimental-strip-types --check` for syntax and by hand against the same logic already proven correct on the bash side | A user reports the no-Git-Bash case in practice, or Claude Code adds a supported per-OS hook command field |
| D13 | `plugins/checkbox/.mcp.json`'s hardcoded `venv/bin/python3` stays as-is for Windows; no in-repo fix attempted. `checkbox doctor`'s `mcp_venv` check gains a Windows-only note naming the actual `venv\Scripts\python.exe` path and telling the user to register `checkbox-mcp` in their own project/user MCP config instead | Investigated a `"command": "python3"` (bare, PATH-resolved) bootstrap that would re-exec into the OS-correct venv path computed in Python (`cli.py`'s existing `sys.platform` ternary) -- rejected because it inherits D12's exact problem: on the Windows machine D12 was verified against, nothing on PATH answered to `python3`/`python` except the Store alias stub, which can't run the re-exec logic that would fix it. `py` (the Windows Python Launcher, D12's actual working fallback) is Windows-only naming, so no single command string is uniform across OSes. D12's fix doesn't transfer because MCP server commands are spawned with no shell and Claude Code has no per-OS command field (verified against code.claude.com/docs, 2026-09-17) -- unlike `hooks.json`, there's no shell-form escape hatch to run a `$PATH`-walk in first | Claude Code adds a supported per-OS MCP command field, or a real Windows non-Claude-host user needs `checkbox-mcp` badly enough to justify a manual per-machine `.mcp.json` edit becoming a documented setup step instead of a `doctor` note |
