...
| File | Host(s) | Notes |
|---|---|---|
| `AGENTS.md` | OpenCode, Codex, Amp, Jules and other AGENTS.md readers | Full ladder rendered with neutral placeholders, plus "run `checkbox profile show` to read this project's profile" |
| `cursor/checkbox.mdc` | Cursor | Always-apply rule |
| `windsurf/checkbox.md` | Windsurf | Workspace rule |
...
- **Rendering.** Profile-specific placeholders render as instructions to read `.checkbox/profile.json` (or to run `checkbox profile show`), never as baked-in values.
