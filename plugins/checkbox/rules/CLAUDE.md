...
- **Placeholders** use `{name}` and are rendered by `lib/checkbox/ladder.py`: `{profile_line}`, `{mode}`, `{version}`, `{edition}`, `{studio_clause}`, `{hosting_clause}`. An unknown placeholder is a test failure.
...
- **Verification.** Every entry must pass:
  ...
  ```bash
  plugins/checkbox/bin/checkbox rules verify --version 18.0 --odoo-src "$ODOO18_SRC" [--enterprise-src "$ODOO18_EE_SRC"]
  ```
