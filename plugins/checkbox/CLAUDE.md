...
- **Windows: won't connect through this file, ever, without a manual step (D13).** MCP server commands are spawned with no shell and Claude Code has no per-OS command field (verified against code.claude.com/docs, 2026-09-17) -- so the hardcoded `venv/bin/python3` above can't be made to resolve to `venv\Scripts\python.exe` from this JSON, and a bare `python3`/`py`/`python` bootstrap doesn't help either, since it inherits the exact Store-stub problem D12 already documents and D12's fix for it needs a shell that `.mcp.json` doesn't have. `checkbox doctor` names the working venv path; a Windows user who wants `checkbox-mcp` registers it themselves in their own project/user MCP config pointed at that path.
